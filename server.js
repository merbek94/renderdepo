...
async function initDatabase() {
  ...
  await pool.query(`
    ...
    -- v24: v22 geçişinde daha önce 0 ile oluşmuş fakat hiç oynanmamış oyun satırlarına
    -- olması gereken oyun-başı 50 puanı bir kez uygula. Gerçekten oynanmış/istatistikli
    -- satırlara dokunulmaz; böylece maç kaybederek 0'a inmiş puanlar geri şişirilmez.
    DO $per_game_initial50_v24$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM schema_migrations WHERE migration_id = 'per_game_initial50_v24_20260917'
      ) THEN
        UPDATE player_game_progress
        SET general_score = 50,
            monthly_general_score = CASE WHEN monthly_general_score = 0 THEN 50 ELSE monthly_general_score END,
            updated_at = NOW()
        WHERE general_score = 0
          AND infinite_score = 0
          AND infinite_run_score = 0
          AND infinite_next_stage = 1
          AND tournament_stage = 1
          AND tournament_bank = 0
          AND tournament_completed = FALSE
          AND tournament_entry_active = FALSE
          AND hundred_active = FALSE
          AND hundred_stage = 0
          AND two_player_finish_count = 0
          AND two_player_score_count = 0
          AND stats = '{}'::jsonb;

        INSERT INTO schema_migrations (migration_id)
        VALUES ('per_game_initial50_v24_20260917')
        ON CONFLICT (migration_id) DO NOTHING;
      END IF;
    END
    ...
    -- v24: Eski normal sonsuz sistemindeki 5*(1+...+n) yüksek skorlarını doğrudan
    -- "en yüksek aşama = n" biçimine çevir. 729 (üs) ve Rakam Avı (ham skor)
    -- zaten oyun-özel semantik kullandığı için dönüşüm dışında tutulur.
    DO $infinite_high_stage_v24$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM schema_migrations WHERE migration_id = 'infinite_high_stage_v24_20260917'
      ) THEN
        UPDATE player_game_progress
        SET infinite_score = CASE
              WHEN infinite_score <= 0 THEN 0
              ELSE FLOOR((SQRT(1::numeric + (8::numeric * infinite_score::numeric / 5::numeric)) - 1::numeric) / 2::numeric)::bigint
            END,
            monthly_infinite_score = CASE
              WHEN monthly_infinite_score <= 0 THEN 0
              ELSE FLOOR((SQRT(1::numeric + (8::numeric * monthly_infinite_score::numeric / 5::numeric)) - 1::numeric) / 2::numeric)::bigint
            END,
            infinite_run_score = CASE
              WHEN infinite_run_score <= 0 THEN 0
              ELSE FLOOR((SQRT(1::numeric + (8::numeric * infinite_run_score::numeric / 5::numeric)) - 1::numeric) / 2::numeric)::bigint
            END,
            updated_at = NOW()
        WHERE game_key NOT IN ('merge_5120', 'digit_hunt');

        INSERT INTO schema_migrations (migration_id)
        VALUES ('infinite_high_stage_v24_20260917')
        ON CONFLICT (migration_id) DO NOTHING;
      END IF;
    END
    $infinite_high_stage_v24$;
...
function infiniteConsecutiveGridSolved(gridRaw, cfg) {
  const size = cfg.size;
  if (!Array.isArray(gridRaw) || gridRaw.length !== size * size) return false;
  const grid = gridRaw.map(Number);
  if (!grid.every((value) => Number.isInteger(value) && value > 0 && value <= 9999)) return false;
  const rowSteps = [];
  const colSteps = [];
  for (let row = 0; row < size; row += 1) {
    const first = grid[row * size];
    const step = grid[row * size + 1] - first;
    if (row === 0 && (first < cfg.startMin || first > cfg.startMax)) return false;
    if (step < cfg.stepMin || step > cfg.stepMax) return false;
    for (let col = 2; col < size; col += 1) if (grid[row * size + col] - grid[row * size + col - 1] !== step) return false;
    rowSteps.push(step);
  }
  for (let col = 0; col < size; col += 1) {
    const step = grid[size + col] - grid[col];
    if (step < cfg.stepMin || step > cfg.stepMax) return false;
    for (let row = 2; row < size; row += 1) if (grid[row * size + col] - grid[(row - 1) * size + col] !== step) return false;
    colSteps.push(step);
  }
...
function generateInfiniteConsecutivePuzzle(stageValue) {
  ...
  throw new Error("Sonsuz Ardışık bulmacası üretilemedi.");
}
...
function generateEquationHuntPuzzle(forcedFamily = null) {
  ...
  const family = Number.isInteger(forcedFamily) && forcedFamily >= 1 && forcedFamily <= 7
    ? forcedFamily
    : secureRandomInt(1, 8);
  ...
  // Zorlanmış aile kullanılıyorsa fallback de aynı ailede kalır; böylece Sonsuz Mod
  // aşama kuralı nadir üretim başarısızlığında bile bozulmaz.
  if (family === 3) {
    // 5x - 3 = 2x + 15 => x=6; 4y + 1 = y + 13 => y=4.
    return {
      gameKey: "equation_hunt",
      difficulty: "Standard",
      target: 6,
      numbers: [3, 0, 4, 5, -3, 2, 15, 4, 1, 1, 13],
      initialGrid: [],
    };
  }
  if (family === 7) {
    // x² + 1 = 10 => x=3; y² + 2 = 18 => y=4.
    return {
      gameKey: "equation_hunt",
      difficulty: "Standard",
      target: 3,
      numbers: [7, 0, 4, 0, 1, 1, 0, 1, 2],
      initialGrid: [],
    };
  }

  // Varsayılan güvenli fallback: 2x + 5 = 11, 3y - 4 = 8 => x=3, y=4.
  return {
    gameKey: "equation_hunt",
    difficulty: "Standard",
    target: 3,
...
function generateInfiniteEquationSystemPuzzle(variableCount, independentOneSide = false, coefficientMax = 6) {
  ...
  for (let attempt = 0; attempt < 5000; attempt += 1) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const rightConst = independentOneSide
        ? leftValue
        : leftValue - rightCoeff * solutions[rightVar];
      if (!Number.isInteger(rightConst) || Math.abs(rightConst) > 80) { equations.length = 0; break; }
      if (!independentOneSide && rightCoeff * solutions[rightVar] + rightConst <= 0) { equations.length = 0; break; }
      equations.push(leftVar, leftCoeff, leftConst, rightVar, rightCoeff, rightConst);
      leftProduct *= leftCoeff;
      if (!independentOneSide) rightProduct *= rightCoeff;
    }
    if (equations.length !== n * 6) continue;
    // Döngüsel sistemde katsayı matrisi tekil olmasın. Bağımsız tek-taraflı sistem zaten tekildir değildir.
    if (!independentOneSide && leftProduct === rightProduct) continue;
    return {
      gameKey: "equation_hunt",
      difficulty: "Standard",
      target: solutions[0],
      // Aile 8: [8, n, çözüm2..n, her denklem için leftVar,leftCoeff,leftConst,rightVar,rightCoeff,rightConst]
      numbers: [8, n, ...solutions.slice(1), ...equations],
      initialGrid: [],
    };
  }
  throw new Error("Sonsuz Denklem Avı çok bilinmeyenli sistem üretilemedi.");
}

function generateInfiniteEquationHuntPuzzle(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  // 1-10: tek bilinmeyenli, bilinmeyen yalnız bir tarafta (ax ± b = c).
...
function isEquationHuntPuzzleEncodingValid(puzzle) {
  ...
  if (family < 1 || family > 8) return false;
  if (family !== 8 && (pairIndex < 0 || pairIndex > 2)) return false;

  const maxSolution = family === 7
    ? EQUATION_HUNT_MAX_QUADRATIC_SOLUTION
    : EQUATION_HUNT_MAX_LINEAR_SOLUTION;
  if (firstSolution < 1 || firstSolution > maxSolution) return false;
  if (family !== 8 && (secondSolution < 1 || secondSolution > maxSolution)) return false;
  ...
  if (family === 8) {
    const variableCount = numbers[1];
    if (!Number.isInteger(variableCount) || variableCount < 2 || variableCount > 4) return false;
    const expectedLength = 7 * variableCount + 1;
    if (numbers.length !== expectedLength) return false;
    const solutions = [firstSolution, ...numbers.slice(2, 2 + variableCount - 1)];
    if (solutions.length !== variableCount || solutions.some((value) => !Number.isInteger(value) || value < 1 || value > EQUATION_HUNT_MAX_LINEAR_SOLUTION)) return false;
    const start = variableCount + 1;
    for (let equation = 0; equation < variableCount; equation += 1) {
      const offset = start + equation * 6;
      const leftVar = numbers[offset];
      const leftCoeff = numbers[offset + 1];
      const leftConst = numbers[offset + 2];
      const rightVar = numbers[offset + 3];
...
function validateEquationHuntChallengeAnswer(puzzle, answer = {}) {
  ...
  const family = Number(puzzle?.numbers?.[0]);
  if (family === 8) {
    const variableCount = Number(puzzle.numbers[1]);
    const expected = [Number(puzzle.target), ...puzzle.numbers.slice(2, 2 + variableCount - 1).map(Number)];
    const values = Array.isArray(answer?.values) ? answer.values.map(Number) : [];
    return values.length === variableCount && values.every((value, index) => Number.isInteger(value) && value === expected[index]);
  }

  const firstValue = Number(answer?.firstValue);
  if (!Number.isInteger(firstValue)) return false;

  // Sonsuz Mod 1-20. aşamalar tek bilinmeyenli/tek denklemlidir. İkinci gizli
  // parametre yalnız ortak puzzle codec uyumluluğu için taşınır ve cevapta aranmaz.
  const infiniteStage = Math.max(0, Math.floor(Number(puzzle?.infiniteStage || 0)));
  if (infiniteStage >= 1 && infiniteStage <= 20) {
    return firstValue === Number(puzzle.target);
  }

  const secondValue = Number(answer?.secondValue);
  if (!Number.isInteger(secondValue)) return false;
  return firstValue === Number(puzzle.target) &&
...
function challengeRewards(mode, stage) {
  ...
  if (mode === "infinite") {
    // 729 ve Rakam Avı aşağıdaki oyun-özel dallarda ele alınır.
    // Diğer oyunlarda sonsuz puan artık ulaşılan en yüksek aşamadır; XP ise
    // yalnız kişisel rekor aşaması geçildiğinde sabit 20 verilir.
    return { generalDelta: 0, infiniteDelta: safeStage, xpDelta: 20 };
  }
  return { generalDelta: 0, infiniteDelta: 0, xpDelta: 0 };
}

const TOURNAMENT_STAGE_REWARDS = [50, 150, 350, 1000, 2500, 7000, 18000, 50000];
const TOURNAMENT_STAGE_XP_REWARDS = [50, 100, 150, 200, 250, 300, 350, 400];
...
function tournamentStageXpReward(stageValue) {
  const stage = Math.max(1, Math.min(Number(stageValue || 1), TOURNAMENT_STAGE_XP_REWARDS.length));
  return TOURNAMENT_STAGE_XP_REWARDS[stage - 1];
}
...
async function consumeGameRightInTransaction(client, playerId, difficulty, wagerPoints = minimumTwoPlayerStake(difficulty), allowAutomatic = false, gameKey = "target_number") {
  ...
  state.finishProfile = normalizeTwoPlayerFinishProfile({
    ...
    botGameCount: lockedRow.bot_timing_game_count,
...
async function applyTournamentOutcomeInTransaction(
  ...
  if (won === true) {
    ...
    xpDelta = tournamentStageXpReward(currentStage);
...
function normalizeTwoPlayerFinishProfile(profile = {}) {
  ...
  const botGameCount = Number.isFinite(parsedBotGameCount)
    ? Math.max(0, Math.min(2_000_000_000, Math.floor(parsedBotGameCount)))
    : 0;
  return { finishCount, finishTotalMs, averageFinishMs, scoreCount, scoreTotal, averageScore, generalScore, botGameCount };
}
...
async function readTwoPlayerFinishProfileInTransaction(client, playerId, gameKey = "target_number") {
  ...
  return normalizeTwoPlayerFinishProfile({
    ...
    botGameCount: row.bot_timing_game_count,
...
async function incrementAdaptiveBotTimingGameInTransaction(client, playerId, gameKey) {
  const base = normalizeBaseGameKey(gameKey);
  if (base !== "merge_5120" && base !== "digit_hunt") return;
  await ensurePlayerGameProgress(client, playerId, base);
  await client.query(
    `UPDATE player_game_progress
     SET bot_timing_game_count = LEAST(bot_timing_game_count + 1, 2000000000),
         updated_at = NOW()
     WHERE player_id = $1 AND game_key = $2`,
    [playerId, base]
  );
...
async function incrementAdaptiveBotTimingGame(playerId, gameKey) {
  if (!pool || !playerId) return;
  const base = normalizeBaseGameKey(gameKey);
  if (base !== "merge_5120" && base !== "digit_hunt") return;
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await incrementAdaptiveBotTimingGameInTransaction(client, playerId, base);
    await client.query("COMMIT");
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
...
function isScoreBasedGameKey(gameKey) {
  const base = normalizeBaseGameKey(gameKey);
  // Legacy/100 kişilik ve sonsuz sonuç alanlarında bu iki oyun skor verisi taşımaya devam eder.
  return base === "merge_5120" || base === "digit_hunt";
}

function isRealtimeScoreBasedGameKey(gameKey) {
  normalizeBaseGameKey(gameKey);
  // Normal ikili/arkadaş/turnuva gerçek-zamanlı yarışta artık skor karşılaştırması yoktur.
  return false;
}

function isUntimedSingleRunGameKey(gameKey) {
  const base = normalizeBaseGameKey(gameKey);
  return base === "merge_5120" || base === "digit_hunt" || base === "digit_attack";
}

function isFirstFinishRaceGameKey(gameKey) {
  const base = normalizeBaseGameKey(gameKey);
  return base === "merge_5120" || base === "digit_hunt";
}
...
function digitAttackDecodeWaves(numbersRaw) {
  ...
  if (numbers.length < DIGIT_ATTACK_WAVE_STRIDE || numbers.length % DIGIT_ATTACK_WAVE_STRIDE !== 0) return null;
  const waves = [];
...
function isDigitAttackPuzzleEncodingValid(puzzle) {
  ...
  if (Number(puzzle?.target) !== (infiniteRun ? waves.length : DIGIT_ATTACK_REQUIRED_HITS)) return false;
...
function digitAttackAdvancedNearbyOperands(base, operation, correctOperand, target) {
  const radius = operation === 0 || operation === 1 ? 10 : 5;
  const minimum = operation === 0 || operation === 1 ? 5 : 2;
  const candidates = shuffled(Array.from({ length: radius * 2 + 1 }, (_,i)=>i-radius).filter((d)=>d!==0));
  const result=[correctOperand];
  for (const delta of candidates) {
    const candidate=correctOperand+delta;
    if (candidate < minimum || result.includes(candidate)) continue;
    if (digitAttackApply(base, operation, candidate) === target) continue;
    result.push(candidate);
    if (result.length===3) break;
  }
...
function generateInfiniteDigitAttackWave(base, advanced) {
  const operations = shuffled([0,1,2,3]);
  for (const operation of operations) {
    const minCorrect = operation <= 1 ? 9 : 2;
    const maxCorrect = advanced ? (operation <= 1 ? 50 : 8) : (operation <= 1 ? 25 : 5);
...
function infiniteShortestPathDimensions(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
...
function shortestPathDynamicHorizontalIndex(rows, cols, row, col) { return row * (cols - 1) + col; }
function shortestPathDynamicVerticalIndex(rows, cols, row, col) { return rows * (cols - 1) + row * cols + col; }
function shortestPathDynamicEdgeWeight(numbers, rows, cols, a, b) {
  const ar = Math.floor(a / cols), ac = a % cols, br = Math.floor(b / cols), bc = b % cols;
  let index = -1;
  if (ar === br && Math.abs(ac - bc) === 1) index = shortestPathDynamicHorizontalIndex(rows, cols, ar, Math.min(ac, bc));
  else if (ac === bc && Math.abs(ar - br) === 1) index = shortestPathDynamicVerticalIndex(rows, cols, Math.min(ar, br), ac);
  return index >= 0 ? Number(numbers[index]) : null;
}
function shortestPathDynamicRoutes(rows, cols) {
  const end = rows * cols - 1, out = [];
  function walk(node, path) {
    if (node === end) { out.push(path); return; }
    const row = Math.floor(node / cols), col = node % cols;
    if (col + 1 < cols) walk(node + 1, [...path, node + 1]);
    if (row + 1 < rows) walk(node + cols, [...path, node + cols]);
  }
  walk(0, [0]);
  return out;
}
function shortestPathDynamicTotal(numbers, rows, cols, route) {
  const expectedLength = rows + cols - 1;
  if (!Array.isArray(route) || route.length !== expectedLength || route[0] !== 0 || route[route.length - 1] !== rows * cols - 1) return null;
  let total = 0;
  for (let i = 0; i < route.length - 1; i += 1) {
    const a = route[i], b = route[i + 1];
    const ar = Math.floor(a / cols), ac = a % cols, br = Math.floor(b / cols), bc = b % cols;
    if (!((br === ar && bc === ac + 1) || (br === ar + 1 && bc === ac))) return null;
    const weight = shortestPathDynamicEdgeWeight(numbers, rows, cols, a, b);
    if (!Number.isInteger(weight)) return null;
    total += weight;
  }
  return total;
}
function generateInfiniteShortestPathPuzzle(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  const { rows, cols } = infiniteShortestPathDimensions(stage);
  const edgeCount = shortestPathDynamicEdgeCount(rows, cols);
  const routes = shortestPathDynamicRoutes(rows, cols);
  const base = Array.from({ length: edgeCount }, (_, i) => i + 1);
  for (let attempt = 0; attempt < 1500; attempt += 1) {
    const numbers = shuffled(base);
    const totals = routes.map((route) => ({ route, total: shortestPathDynamicTotal(numbers, rows, cols, route) }));
    const minimum = Math.min(...totals.map((item) => item.total));
    if (totals.filter((item) => item.total === minimum).length !== 1) continue;
    return { difficulty: "Standard", target: minimum, numbers, gameKey: "shortest_path", initialGrid: [], infiniteStage: stage };
  }
  throw new Error("Sonsuz En Kısa Yol bulmacası üretilemedi.");
}
function validateInfiniteShortestPathAnswer(puzzle, answer = {}) {
  const { rows, cols } = infiniteShortestPathDimensions(puzzle?.infiniteStage);
  const edgeCount = shortestPathDynamicEdgeCount(rows, cols);
  const numbers = Array.isArray(puzzle?.numbers) ? puzzle.numbers.map(Number) : [];
  if (numbers.length !== edgeCount || [...numbers].sort((a,b)=>a-b).some((v,i)=>v !== i+1)) return false;
  const route = Array.isArray(answer?.route) ? answer.route.map(Number) : [];
  return shortestPathDynamicTotal(numbers, rows, cols, route) !== null;
}
function infiniteShortestPathAnswerIsWinning(puzzle, answer = {}) {
  const { rows, cols } = infiniteShortestPathDimensions(puzzle?.infiniteStage);
  const route = Array.isArray(answer?.route) ? answer.route.map(Number) : [];
  return shortestPathDynamicTotal(puzzle?.numbers, rows, cols, route) === Number(puzzle?.target);
}
...
// ============================================================================
// 729 / SAYI BULMACASI / SUDOKU / NONOGRAM / SONUCU BUL authoritative rules
// ============================================================================
const MERGE_5120_ROWS = 8;
const MERGE_5120_COLS = 6;
const MERGE_5120_CELLS = MERGE_5120_ROWS * MERGE_5120_COLS;
// Sonsuz 729'da toplam hamle sayısına yapay bir üst sınır yoktur.
// Uzun koşular secure_game_challenges.progress_state içinde kompakt authoritative checkpoint olarak tutulur.
...
function merge5120DropState(state, seed, columnValue) {
  ...
  merge5120Gravity(board);
  return { board, moveCount: Number(state.moveCount || 0) + 1, score, maxTileEver };
}
...
function replayMerge5120(puzzle, answer = {}) {
  ...
  let state = { board: Array(MERGE_5120_CELLS).fill(null), moveCount: 0, score: 0, maxTileEver: 0 };
...
function merge5120ApplyCheckpointMoves(puzzle, checkpointRaw, baseMoveCountValue, movesRaw) {
  const seed = Number(puzzle?.numbers?.[0]);
  if (!Number.isInteger(seed)) return null;
  const moves = Array.isArray(movesRaw) ? movesRaw.map(Number) : [];
  if (moves.some((move) => !Number.isInteger(move) || move < 0 || move >= MERGE_5120_COLS)) return null;
  ...
  for (let index = alreadyApplied; index < moves.length; index += 1) {
    const next = merge5120DropState(state, seed, moves[index]);
    if (!next) return null;
    state = next;
  }
  return state;
...
function merge5120AnswerMatchesState(state, answer = {}) {
  ...
  const totalMoveCount = Number(answer?.totalMoveCount ?? state.moveCount);
  return Number.isSafeInteger(totalMoveCount) && totalMoveCount === Number(state.moveCount || 0);
}
...
function generateInfiniteNumberPuzzle(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  const fixedCount = stage <= 40 ? 8 : stage <= 80 ? 6 : stage <= 120 ? 4 : 2;
  const puzzle = generateNumberPuzzle();
  const numberCells = puzzle.numbers.map((value, index) => value > 0 ? index : -1).filter((index) => index >= 0);
  const fixed = new Set(shuffled(numberCells).slice(0, Math.min(fixedCount, numberCells.length)));
  const initialGrid = puzzle.numbers.map((value, index) => value > 0 ? (fixed.has(index) ? value : null) : value);
  return { ...puzzle, initialGrid, infiniteStage: stage };
}
...
function sudoku9Pattern(row, col) { return (row * 3 + Math.floor(row / 3) + col) % 9; }
function generateSudoku9Solution() {
  const groups = (groupSize, groupCount) => shuffled(Array.from({ length: groupCount }, (_, i) => i)).flatMap((group) =>
    shuffled(Array.from({ length: groupSize }, (_, i) => i)).map((inner) => group * groupSize + inner));
  const rows = groups(3, 3), cols = groups(3, 3), digits = shuffled(Array.from({ length: 9 }, (_, i) => i + 1));
  return rows.flatMap((r) => cols.map((c) => digits[sudoku9Pattern(r, c)]));
}
function sudoku9SolutionIsValid(solution) {
  if (!Array.isArray(solution) || solution.length !== 81) return false;
  const validSet = (arr) => arr.length === 9 && arr.every((v) => Number.isInteger(Number(v)) && Number(v) >= 1 && Number(v) <= 9) && new Set(arr.map(Number)).size === 9;
  for (let r = 0; r < 9; r += 1) if (!validSet(solution.slice(r * 9, r * 9 + 9))) return false;
  for (let c = 0; c < 9; c += 1) if (!validSet(Array.from({ length: 9 }, (_, r) => solution[r * 9 + c]))) return false;
  for (let br = 0; br < 3; br += 1) for (let bc = 0; bc < 3; bc += 1) {
    const vals = [];
    for (let r = 0; r < 3; r += 1) for (let c = 0; c < 3; c += 1) vals.push(solution[(br * 3 + r) * 9 + bc * 3 + c]);
    if (!validSet(vals)) return false;
  }
  return true;
}
function sudoku9ClueSpec(stageValue) {
  const stage = Math.max(41, Math.floor(Number(stageValue || 41)));
  if (stage <= 80) return { clues: 35, maxPerLine: 4 };
  if (stage <= 120) return { clues: 26, maxPerLine: 3 };
  return { clues: 18, maxPerLine: 2 };
}
function sudoku9CluePositions(clues, maxPerLine) {
  const offsets = shuffled([0,1,2,3,4,5,6,7,8]).slice(0, maxPerLine);
  const positions = [];
  for (let layer = 0; layer < maxPerLine; layer += 1) {
    for (let row = 0; row < 9; row += 1) positions.push(row * 9 + ((row + offsets[layer]) % 9));
  }
  while (positions.length > clues) positions.splice(secureRandomInt(0, positions.length), 1);
  return positions;
}
function generateInfiniteSudokuPuzzle(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 40) return { ...generateSudokuPuzzle(), infiniteStage: stage };
  const spec = sudoku9ClueSpec(stage);
  const solution = generateSudoku9Solution();
  const fixed = new Set(sudoku9CluePositions(spec.clues, spec.maxPerLine));
  const initialGrid = solution.map((value, index) => fixed.has(index) ? value : null);
  return { difficulty: "Standard", target: 81, numbers: solution, gameKey: "sudoku", initialGrid, infiniteStage: stage };
}
function validateInfiniteSudokuAnswer(puzzle, answer = {}) {
  const stage = Math.max(1, Math.floor(Number(puzzle?.infiniteStage || 1)));
  if (stage <= 40) return validateSudokuAnswer(puzzle, answer);
  const spec = sudoku9ClueSpec(stage);
  const solution = Array.isArray(puzzle?.numbers) ? puzzle.numbers.map(Number) : [];
  const initialGrid = Array.isArray(puzzle?.initialGrid) ? puzzle.initialGrid : [];
  if (Number(puzzle?.target) !== 81 || !sudoku9SolutionIsValid(solution) || initialGrid.length !== 81) return false;
  const fixedIndices = initialGrid.map((v,i)=>v==null?-1:i).filter((i)=>i>=0);
  if (fixedIndices.length !== spec.clues) return false;
  const rowCounts = Array(9).fill(0), colCounts = Array(9).fill(0);
  for (const i of fixedIndices) {
    if (Number(initialGrid[i]) !== solution[i]) return false;
    rowCounts[Math.floor(i/9)] += 1; colCounts[i%9] += 1;
  }
  if (rowCounts.some((c)=>c>spec.maxPerLine) || colCounts.some((c)=>c>spec.maxPerLine)) return false;
  const grid = Array.isArray(answer?.grid) ? answer.grid.map(Number) : [];
  if (!sudoku9SolutionIsValid(grid)) return false;
  for (const i of fixedIndices) if (grid[i] !== Number(initialGrid[i])) return false;
  return true;
}
...
function infiniteResultFindConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 20) return { count: 3, valueMax: 10, resultMax: 99, parens: 0, parenMulMax: 0, outsideMulMax: 999 };
  if (stage <= 40) return { count: 5, valueMax: 15, resultMax: 99, parens: 0, parenMulMax: 0, outsideMulMax: 999 };
  if (stage <= 60) return { count: 7, valueMax: 20, resultMax: 199, parens: 1, parenMulMax: 5, outsideMulMax: 12 };
  if (stage <= 80) return { count: 9, valueMax: 30, resultMax: 299, parens: 2, parenMulMax: 7, outsideMulMax: 16 };
  if (stage <= 100) return { count: 11, valueMax: 40, resultMax: 399, parens: 2, parenMulMax: 9, outsideMulMax: 20 };
  return { count: 13, valueMax: 50, resultMax: 499, parens: 3, parenMulMax: 12, outsideMulMax: 30 };
}
function resultFindRangesValidForConfig(nums, ops, ranges, cfg) {
  ...
  for (let i = 0; i < ops.length; i += 1) {
    if (ops[i] !== 2) continue;
    const touchesParen = ranges.some(([a,b]) => i === a-1 || i === b);
    if (!touchesParen && (nums[i] > cfg.outsideMulMax || nums[i+1] > cfg.outsideMulMax)) return false;
  }
  return true;
}
function generateInfiniteResultFindPuzzle(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  const cfg = infiniteResultFindConfig(stage);
  for (let attempt = 0; attempt < 12000; attempt += 1) {
    const nums = Array.from({ length: cfg.count }, () => secureRandomInt(2, cfg.valueMax + 1));
    const ops = Array.from({ length: cfg.count - 1 }, () => secureRandomInt(0, 4));
    const ranges = [];
    if (cfg.parens > 0) {
      const starts = shuffled(Array.from({ length: cfg.count - 1 }, (_,i)=>i)).filter((start) => start !== 4);
      for (const start of starts) {
        if (ranges.length >= cfg.parens) break;
        if (ranges.some(([a,b]) => Math.abs(start-a) <= 2 || Math.abs(start-b) <= 1)) continue;
        ranges.push([start,start+1]);
      }
      if (ranges.length !== cfg.parens) continue;
    ...
    for (let i = 0; i < ops.length; i += 1) {
      const touchesParen = ranges.some(([a,b]) => i === a-1 || i === b);
      if (!touchesParen && ops[i]===2 && (nums[i] > cfg.outsideMulMax || nums[i+1] > cfg.outsideMulMax)) {
        if (nums[i] > cfg.outsideMulMax) nums[i] = secureRandomInt(2, cfg.outsideMulMax+1);
        if (nums[i+1] > cfg.outsideMulMax) nums[i+1] = secureRandomInt(2, cfg.outsideMulMax+1);
      }
    }
  ...
  throw new Error("Sonsuz Sonucu Bul bulmacası üretilemedi.");
}
...
function infiniteTotalMatchTargetRange(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 40) return [6,20];
  if (stage <= 80) return [20,40];
  if (stage <= 120) return [40,60];
  return [50,100];
}
function generateInfiniteTotalMatchPuzzle(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  const [minTarget,maxTarget] = infiniteTotalMatchTargetRange(stage);
  const target = secureRandomInt(minTarget, maxTarget + 1);
  const values = [];
  for (let pair=0; pair<28; pair+=1) {
    const left = secureRandomInt(1, target);
    values.push(left, target-left);
  }
  return { difficulty:"Standard", target, numbers:shuffled(values), gameKey:"total_match", initialGrid:[], infiniteStage:stage };
}
function validateInfiniteTotalMatchAnswer(puzzle, answer = {}) {
  const [minTarget,maxTarget] = infiniteTotalMatchTargetRange(puzzle?.infiniteStage);
  const target = Number(puzzle?.target), numbers = Array.isArray(puzzle?.numbers) ? puzzle.numbers.map(Number) : [];
  if (!Number.isInteger(target) || target < minTarget || target > maxTarget || numbers.length !== 56) return false;
  const counts = new Map();
  ...
  for (const [value,count] of counts.entries()) {
    ...
    if (value===comp ? count%2!==0 : (counts.get(comp)||0)!==count) return false;
  }
  const remaining = Array.isArray(answer?.remaining) ? answer.remaining : [];
  return remaining.length===56 && remaining.every((value)=>value==null);
}
...
function validateWrongNumbersAnswer(puzzle, answer = {}) {
  ...
  const solution = puzzle.numbers.slice(0, WRONG_NUMBERS_COUNT).map(Number);
  return grid.length === WRONG_NUMBERS_COUNT && grid.every((value, index) => Number.isInteger(Number(value)) && Number(value) === solution[index]);
}

const DIGIT_HUNT_ROWS = 9;
const DIGIT_HUNT_COLS = 9;
const DIGIT_HUNT_CELLS = 81;
const DIGIT_HUNT_TOTAL_TILES = 240;
const DIGIT_HUNT_PER_DIGIT = 48;
...
function digitHuntLineCandidates(board, indices, orientationOrder) {
  ...
  while (start < values.length) {
    ...
    if (end - start >= 3) {
      // 4/5/... eş taşta tek çözüm adımında yalnız ilk üç taş silinir.
      result.push({ indices: indices.slice(start, start + 3), order: orientationOrder });
    }
    start = end;
  }
  return result;
...
function digitHuntApplyMoveState(state, from, to, seed, initialBoard, unlimitedSpawns, requireMatchMode) {
  ...
  const mustCreateMatch = requireMatchMode || !state.board.some((v) => v == null);
...
function digitHuntHasLegalMove(board, seed = 1, spawnCounter = 0, initialBoard = board, isInfiniteMode = true) {
  ...
  if (!isInfiniteMode) {
    if (!board.some((v) => v == null)) {
      // Dolu ızgarada yalnız gerçek üçlü üreten komşu takas geçerlidir.
      for (let i = 0; i < 81; i += 1) {
        const r = Math.floor(i / 9), c = i % 9;
        if (c < 8 && digitHuntApplyMoveState({ board, score: 0, spawnCounter }, i, i + 1, seed, initialBoard, false, false)) return true;
        if (r < 8 && digitHuntApplyMoveState({ board, score: 0, spawnCounter }, i, i + 9, seed, initialBoard, false, false)) return true;
      }
      return false;
    }
...
function generateDigitHuntPuzzle() {
  for (let attempt = 0; attempt < 1000; attempt += 1) {
    ...
    const createsRunAt = (index, value) => {
      ...
      if (col >= 2) { const a=board[index-2],b=board[index-1]; if (a===b&&b===value) return true; }
      if (row >= 2) { const a=board[index-18],b=board[index-9]; if (a===b&&b===value) return true; }
      return false;
    };
    let failed = false;
    for (let i=0;i<81;i+=1) {
...
// Her oyun kendi zorluk bantlarını kullanır; bazı oyunlar 20, bazıları 30 veya 40 aşamalık bantlara ayrılır.
// ============================================================================
...
function generateInfiniteTargetNumberPuzzle(difficultyValue, stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  const cfg = stage <= 40
    ? { count: 3, numberMin: 2, numberMax: 10, targetMin: 0, targetMax: 80 }
    : stage <= 80
      ? { count: 4, numberMin: 2, numberMax: 10, targetMin: 1, targetMax: 200 }
      : stage <= 120
...
function infiniteEqualSumConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 40) return { size: 3, minTarget: 12, maxTarget: 20, fixedCount: 4, maxFixedPerLine: 2 };
  if (stage <= 80) return { size: 4, minTarget: 20, maxTarget: 40, fixedCount: 8, maxFixedPerLine: 3 };
  if (stage <= 120) return { size: 5, minTarget: 30, maxTarget: 60, fixedCount: 12, maxFixedPerLine: 4 };
  return { size: 6, minTarget: 40, maxTarget: 80, fixedCount: 11, maxFixedPerLine: 2 };
}
...
function chooseInfiniteEqualSumFixedIndices(size, fixedCount, maxFixedPerLine) {
  const total = size * size;
  const cap = Math.max(1, Math.min(size - 1, Number(maxFixedPerLine || size - 1)));
  for (let attempt = 0; attempt < 5000; attempt += 1) {
    const rows = Array(size).fill(0);
    const cols = Array(size).fill(0);
    const selected = [];
    for (const index of shuffled(Array.from({ length: total }, (_, i) => i))) {
      const row = Math.floor(index / size), col = index % size;
      if (rows[row] >= cap || cols[col] >= cap) continue;
      selected.push(index); rows[row] += 1; cols[col] += 1;
      if (selected.length === fixedCount) return new Set(selected);
    }
  }
  return null;
}
...
function generateInfiniteEqualSumPuzzle(stageValue) {
  ...
  for (let attempt = 0; attempt < 500; attempt += 1) {
    ...
    if (!solution) continue;
    const fixedCount = cfg.fixedCount;
    const fixed = chooseInfiniteEqualSumFixedIndices(cfg.size, fixedCount, cfg.maxFixedPerLine);
    if (!fixed) continue;
    const initialGrid = solution.map((value, index) => fixed.has(index) ? value : null);
...
function validateInfiniteEqualSumAnswer(puzzle, numberSlotsRaw) {
  ...
  if (initialGrid.length !== total || pool.length !== total - cfg.fixedCount || slots.length !== total) return false;
  const fixedRows = Array(size).fill(0), fixedCols = Array(size).fill(0);
  initialGrid.forEach((value, index) => {
    if (value != null) {
      fixedRows[Math.floor(index / size)] += 1;
      fixedCols[index % size] += 1;
    }
  });
  if (initialGrid.filter((v) => v != null).length !== cfg.fixedCount) return false;
  if (fixedRows.some((c) => c > cfg.maxFixedPerLine) || fixedCols.some((c) => c > cfg.maxFixedPerLine)) return false;
...
function infiniteArithmeticConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 40) return { targetCount: 2, perTarget: 2, valueMin: 2, valueMax: 10, targetMin: 2, targetMax: 7, fixedCount: 2, maxFixedPerTarget: 1 };
  if (stage <= 80) return { targetCount: 3, perTarget: 3, valueMin: 2, valueMax: 34, targetMin: 11, targetMax: 25, fixedCount: 3, maxFixedPerTarget: 1 };
  if (stage <= 120) return { targetCount: 4, perTarget: 4, valueMin: 4, valueMax: 48, targetMin: 16, targetMax: 36, fixedCount: 6, maxFixedPerTarget: 2 };
  return { targetCount: 5, perTarget: 4, valueMin: 6, valueMax: 72, targetMin: 24, targetMax: 54, fixedCount: 7, maxFixedPerTarget: 2 };
}
...
function chooseInfiniteArithmeticFixedIndices(cfg) {
  const selected = [];
  const perGroup = Array(cfg.targetCount).fill(0);
  // Önce gruplara olabildiğince dengeli birer taş dağıt; sonra ikinci tura geç.
  for (let round = 0; round < cfg.maxFixedPerTarget && selected.length < cfg.fixedCount; round += 1) {
    for (const group of shuffled(Array.from({ length: cfg.targetCount }, (_, i) => i))) {
      if (selected.length >= cfg.fixedCount) break;
      if (perGroup[group] >= cfg.maxFixedPerTarget) continue;
      const candidates = Array.from({ length: cfg.perTarget }, (_, local) => group * cfg.perTarget + local)
        .filter((index) => !selected.includes(index));
      if (candidates.length === 0) continue;
      selected.push(candidates[secureRandomInt(0, candidates.length)]);
      perGroup[group] += 1;
    }
  }
...
function generateInfiniteTotalEqualsPuzzle(stageValue) {
  ...
  const values = groups.flat();
  const fixed = chooseInfiniteArithmeticFixedIndices(cfg);
...
function validateInfiniteTotalEqualsAnswer(puzzle, answer = {}) {
  ...
  for (let group = 0; group < cfg.targetCount; group += 1) {
    const fixedInGroup = initialGrid.slice(group * cfg.perTarget, (group + 1) * cfg.perTarget).filter((v) => v != null).length;
    if (fixedInGroup > cfg.maxFixedPerTarget) return false;
  }
...
function infiniteTripleBalanceConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 30) return { containers: 2, perContainer: 4, minValue: 1, maxValue: 15 };
  if (stage <= 60) return { containers: 3, perContainer: 5, minValue: 1, maxValue: 20 };
  if (stage <= 90) return { containers: 4, perContainer: 6, minValue: 1, maxValue: 30 };
  if (stage <= 120) return { containers: 5, perContainer: 7, minValue: 1, maxValue: 40 };
  return { containers: 6, perContainer: 8, minValue: 1, maxValue: 50 };
}
...
function infiniteSortConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 30) return { count: 6, operands: 2, resultMin: 1, resultMax: 15, valueMin: 1, valueMax: 30 };
  if (stage <= 60) return { count: 8, operands: 2, resultMin: 10, resultMax: 40, valueMin: 2, valueMax: 30 };
  if (stage <= 90) return { count: 10, operands: 2, resultMin: 20, resultMax: 80, valueMin: 2, valueMax: 50 };
  if (stage <= 120) return { count: 12, operands: 2, resultMin: 30, resultMax: 100, valueMin: 2, valueMax: 40 };
  return { count: 14, operands: 2, resultMin: 40, resultMax: 120, valueMin: 2, valueMax: 50 };
}
...
function infiniteDualPyramidConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 20) return { layers: 3, bottomMin: 1, bottomMax: 7 };
  if (stage <= 40) return { layers: 4, bottomMin: 1, bottomMax: 10 };
  if (stage <= 60) return { layers: 5, bottomMin: 1, bottomMax: 15 };
  if (stage <= 80) return { layers: 6, bottomMin: 1, bottomMax: 20 };
  if (stage <= 100) return { layers: 7, bottomMin: 1, bottomMax: 25 };
  return { layers: 8, bottomMin: 1, bottomMax: 30 };
}
...
function infiniteNextNumberConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 20) return { firstMin: 1, firstMax: 10, xMin: 2, xMax: 2, yMin: 1, yMax: 3, rules: ["multiply_plus","multiply_minus"], variableAllowed: false, parenthesizedMustFixed: true };
  if (stage <= 40) return { firstMin: 1, firstMax: 15, xMin: 2, xMax: 3, yMin: 1, yMax: 5, rules: ["multiply_plus","multiply_minus"], variableAllowed: true, parenthesizedMustFixed: true };
  if (stage <= 60) return { firstMin: 1, firstMax: 20, xMin: 2, xMax: 3, yMin: 1, yMax: 5, rules: ["multiply_plus","multiply_minus","plus_then_multiply","minus_then_multiply"], variableAllowed: true, parenthesizedMustFixed: true, parenthesizedYMax: 2 };
  if (stage <= 80) return { firstMin: 1, firstMax: 25, xMin: 2, xMax: 4, yMin: 1, yMax: 5, rules: ["multiply_plus","multiply_minus","plus_then_multiply","minus_then_multiply"], variableAllowed: true, parenthesizedMustFixed: false };
  if (stage <= 100) return { firstMin: 1, firstMax: 30, xMin: 2, xMax: 5, yMin: 1, yMax: 10, rules: ["multiply_plus","multiply_minus","plus_then_multiply","minus_then_multiply"], variableAllowed: true, parenthesizedMustFixed: false };
  return { firstMin: 1, firstMax: 40, xMin: 2, xMax: 6, yMin: 1, yMax: 10, rules: ["multiply_plus","multiply_minus","plus_then_multiply","minus_then_multiply"], variableAllowed: true, parenthesizedMustFixed: false };
}
...
function infiniteWrongNumbersConfig(stageValue) {
  const stage = Math.max(1, Math.floor(Number(stageValue || 1)));
  if (stage <= 20) return { count: 3, valueMin: 1, valueMax: 8, targetMin: 1, targetMax: 100, parenCount: 0 };
  if (stage <= 40) return { count: 4, valueMin: 2, valueMax: 10, targetMin: 1, targetMax: 100, parenCount: 0 };
  if (stage <= 60) return { count: 5, valueMin: 2, valueMax: 10, targetMin: 1, targetMax: 150, parenCount: 0 };
  if (stage <= 80) return { count: 6, valueMin: 2, valueMax: 15, targetMin: 1, targetMax: 200, parenCount: 1, parenResultMin: 2, parenResultMax: 12, connectorValueMax: 5 };
  if (stage <= 100) return { count: 7, valueMin: 2, valueMax: 20, targetMin: 1, targetMax: 200, parenCount: 1, parenResultMin: 2, parenResultMax: 16, connectorValueMax: 7 };
  return { count: 10, valueMin: 2, valueMax: 25, targetMin: 1, targetMax: 250, parenCount: 2, parenResultMin: 2, parenResultMax: 20, connectorValueMax: 9 };
}
...
function infiniteWrongParenLayoutValid(count, operators, parenStarts, values = null, cfg = null) {
  ...
  for (const start of parenStarts) {
    ...
    const { operatorIndex: connectorIndex, externalValueIndex } = connections[0];
    ...
    // Parantezle irtibatlı dış sayı, diğer tarafında ikinci bir ×/÷ zincirine giremez.
    const otherOperatorIndexes = [externalValueIndex - 1, externalValueIndex]
      .filter((index) => index >= 0 && index < operators.length && index !== connectorIndex);
    if (otherOperatorIndexes.some((index) => Number(operators[index]) === WRONG_NUMBERS_MULTIPLY || Number(operators[index]) === WRONG_NUMBERS_DIVIDE)) return false;
    if (Array.isArray(values) && cfg) {
      const inner = dynamicArithmeticResult([Number(values[start]), Number(values[start + 1])], [Number(operators[start])]);
      if (!Number.isInteger(inner) || inner < cfg.parenResultMin || inner > cfg.parenResultMax) return false;
      if (connectorOp === WRONG_NUMBERS_MULTIPLY) {
        const externalValue = Number(values[externalValueIndex]);
        if (!Number.isInteger(externalValue) || externalValue < 2 || externalValue > cfg.connectorValueMax) return false;
      }
    }
    externalIndexes.push(externalValueIndex);
  }
...
function generateInfiniteWrongNumbersPuzzle(stageValue) {
  ...
  for (let attempt = 0; attempt < 100000; attempt += 1) {
    ...
    if (cfg.parenCount > 0) {
      ...
      for (const start of parenStarts) {
        const connections = infiniteWrongParenExternalConnections(start, parenStarts, cfg.count);
        if (connections.length !== 1) { parenStarts = []; break; }
        const { operatorIndex: connectorIndex, externalValueIndex } = connections[0];
        operators[connectorIndex] = secureRandomInt(2, 4); // yalnız × veya ÷
        // Dış sayının diğer tarafı mutlaka + veya − olsun.
        for (const index of [externalValueIndex - 1, externalValueIndex]) {
          if (index >= 0 && index < operators.length && index !== connectorIndex) operators[index] = secureRandomInt(0, 2);
        }
        // Çarpma bağlantısında dış sayı istenen küçük aralıkta olsun.
        if (operators[connectorIndex] === WRONG_NUMBERS_MULTIPLY) {
          values[externalValueIndex] = randomInclusive(2, cfg.connectorValueMax);
        }
      }
      if (parenStarts.length !== cfg.parenCount) continue;
    }

    if (cfg.parenCount > 0 && !infiniteWrongParenLayoutValid(cfg.count, operators, parenStarts, values, cfg)) continue;
...
function validateInfiniteWrongNumbersAnswer(puzzle, answer = {}) {
  ...
  const numbers = Array.isArray(puzzle?.numbers) ? puzzle.numbers.map(Number) : [];
  const initialGrid = Array.isArray(puzzle?.initialGrid) ? puzzle.initialGrid : [];
  const storedParenCount = cfg.parenCount > 0 ? cfg.parenCount : 1; // parantezsizde -1 sentinel
  const expectedLength = cfg.count + (cfg.count - 1) + storedParenCount;
  ...
  const parenStarts = cfg.parenCount > 0 ? rawParens : [];
  if (!values.every((v) => Number.isInteger(v) && v >= cfg.valueMin && v <= cfg.valueMax)) return false;
  if (!operators.every((op) => Number.isInteger(op) && op >= 0 && op <= 3)) return false;
  if (cfg.parenCount === 0 && !(rawParens.length === 1 && rawParens[0] === -1)) return false;
  if (cfg.parenCount > 0 && (parenStarts.length !== cfg.parenCount || !infiniteWrongParenLayoutValid(cfg.count, operators, parenStarts, values, cfg))) return false;
  if (initialGrid.filter((v) => v == null).length !== 1) return false;
...
function generateInfinitePuzzleForGame(gameKey, difficultyValue, stageValue) {
  ...
  switch (normalizeBaseGameKey(gameKey)) {
    ...
    case "equation_hunt": return generateInfiniteEquationHuntPuzzle(stage);
    case "consecutive": return generateInfiniteConsecutivePuzzle(stage);
    case "shortest_path": return generateInfiniteShortestPathPuzzle(stage);
    case "digit_attack": return generateInfiniteDigitAttackPuzzle();
    case "total_match": return generateInfiniteTotalMatchPuzzle(stage);
    case "number_puzzle": return generateInfiniteNumberPuzzle(stage);
    case "sudoku": return generateInfiniteSudokuPuzzle(stage);
    case "result_find": return generateInfiniteResultFindPuzzle(stage);
    default: {
...
const GAME_HANDLERS = Object.freeze({
  ...
  total_match: Object.freeze({
    ...
    validateAnswer: (puzzle, answer) => Number(puzzle?.infiniteStage || 0) > 0
      ? validateInfiniteTotalMatchAnswer(puzzle, answer)
      : validateTotalMatchAnswer(puzzle, answer),
  }),
  triple_balance: Object.freeze({
    key: "triple_balance",
  ...
  shortest_path: Object.freeze({
    ...
    validateAnswer: (puzzle, answer) => Number(puzzle?.infiniteStage || 0) > 0
      ? validateInfiniteShortestPathAnswer(puzzle, answer)
      : validateShortestPathChallengeAnswer(puzzle, answer),
    isWinningAnswer: (puzzle, answer) => Number(puzzle?.infiniteStage || 0) > 0
      ? infiniteShortestPathAnswerIsWinning(puzzle, answer)
      : shortestPathAnswerIsWinning(puzzle, answer),
  }),
  consecutive: Object.freeze({
    key: "consecutive",
    createPuzzle: () => generateConsecutivePuzzle(),
    validateAnswer: (puzzle, answer) => Number(puzzle?.infiniteStage || 0) > 0
      ? validateInfiniteConsecutiveAnswer(puzzle, answer)
      : validateConsecutiveChallengeAnswer(puzzle, answer),
  }),
  merge_5120: Object.freeze({
    key: "merge_5120",
  ...
  sudoku: Object.freeze({
    ...
    validateAnswer: (puzzle, answer) => Number(puzzle?.infiniteStage || 0) > 0
      ? validateInfiniteSudokuAnswer(puzzle, answer)
      : validateSudokuAnswer(puzzle, answer),
  }),
  nonogram: Object.freeze({
    key: "nonogram",
...
async function ensureAuthenticatedPlayer(client, playerId) {
  ...
  if (existing.rowCount > 0) {
    // Eski hesaplarda bazı oyun satırları v22/v23 geçişinden önce hiç oluşmamış olabilir.
    // Her girişte tek UNNEST INSERT ile eksik oyunları 50 başlangıç puanıyla tamamla.
  ...
  await client.query(
    `INSERT INTO player_game_progress (
     ...
     SELECT $2, game_key, general_score, infinite_score,
            ...
            tournament_entry_active, hundred_active, hundred_stage,
            two_player_finish_count, two_player_finish_total_ms,
            two_player_score_count, two_player_score_total, bot_timing_game_count, stats, NOW()
     ...
     ON CONFLICT (player_id, game_key) DO UPDATE SET
       ...
       bot_timing_game_count = LEAST(
         player_game_progress.bot_timing_game_count::bigint + EXCLUDED.bot_timing_game_count::bigint,
         2000000000
       )::integer,
       stats = CASE
...
app.post("/game/challenges/progress", requireAuth, challengeMutationRateLimit, requireGameplaySession, async (req, res) => {
  if (!requireDatabase(res)) return;
  ...
  try {
    ...
    if (!state) {
      const error = new Error("729 checkpoint hamleleri doğrulanamadı.");
      error.statusCode = 422;
      throw error;
    }
...
app.post("/game/challenges/complete", requireAuth, challengeMutationRateLimit, requireGameplaySession, async (req, res) => {
  ...
  try {
    ...
    const digitAttackInfiniteSingleRun = challenge.mode === "infinite" && gameKey === "digit_attack";
    ...
    if (mergeInfiniteSingleRun) {
      // 729'da klasik skor hesabı yoktur. Sonsuz değer en yüksek 3^n taşının üssü,
      // XP ise yalnız daha önce görülmemiş yeni üsler için 20'şer puandır.
    ...
    if (challenge.mode === "infinite") {
      ...
      if (mergeInfiniteSingleRun) {
        ...
        // Bitişte de aynı rekor kontrolünü yaparak hiçbir 3^n kilometre taşını kaçırma.
        const finalHighExponent = Math.max(0, Number(replayedMergeState?.maxTileEver || 0));
        const newHighExponent = Math.max(oldHighScore, finalHighExponent);
        ...
        const gameScore = Math.max(0, Number(rewards.infiniteDelta || 0));
        const newHighScore = Math.max(oldHighScore, gameScore);
        // Her yeni 1000'lik kişisel-rekor eşiği yalnız bir kez 20 XP verir.
        // Eski rekor aşılmazsa veya yeni bir 1000 eşiği geçilmezse XP yoktur.
        const oldMilestone = Math.floor(oldHighScore / 1000);
        const newMilestone = Math.floor(newHighScore / 1000);
        awardedXp = gameScore > oldHighScore
        ...
        await client.query(
          `UPDATE player_game_progress
           SET infinite_run_score = $3,
               ...
               updated_at = NOW()
           WHERE player_id = $1 AND game_key = $2`,
          [req.auth.sub, gameKey, gameScore]
        );
        ...
        const newHighStage = Math.max(oldHighScore, completedStage);
        // Normal sonsuz oyunlarda sonsuz puan = gelinen en yüksek aşama.
        ...
        await client.query(
          `UPDATE player_game_progress
           SET infinite_run_score = $3,
               ...
               updated_at = NOW()
           WHERE player_id = $1 AND game_key = $2`,
          [req.auth.sub, gameKey, completedStage, completedStage]
        );
        infiniteRunScore = completedStage;
      }
      ...
      const firstFinishBotRace =
        (challenge.mode === "two_player_bot" || challenge.mode === "tournament_bot") &&
        isFirstFinishRaceGameKey(gameKey);
      state = await applyTwoPlayerBotRewardsInTransaction(
        client,
        req.auth.sub,
        rewards,
        {
          finishElapsedMs: firstFinishBotRace
            ? (answerWon ? elapsedServerMs : null)
            : (isScoreBasedGameKey(gameKey) ? null : (answerWon ? elapsedServerMs : null)),
          gameKey
        }
      );
      await recordTaskEventInTransaction(client, {
...
async function recordMergeRealtimeScores(room) {
  if (!pool || !isRealtimeScoreBasedGameKey(room?.gameKey)) return;
...
async function finishRealtimeMatch(room, winner, reason = "rounds_completed") {
  ...
  if (isRealtimeScoreBasedGameKey(room.gameKey)) recordMergeRealtimeScores(room).catch(() => {});
  ...
  roomParticipants(room).forEach((participant) => {
    ...
    emitToRoomParticipant(participant, "match_completed", {
      ...
      myScore: isRealtimeScoreBasedGameKey(room.gameKey) ? Math.max(0, Number(participant.roundScore || 0)) : null,
      opponentScore: isRealtimeScoreBasedGameKey(room.gameKey) ? Math.max(0, Number(opponent?.roundScore || 0)) : null,
    });
  });
}
...
function scheduleRealtimeRound(room, prepareMs = 3_000) {
  ...
  room.deadlineHandle = setTimeout(() => {
    ...
    const scoreBased = isRealtimeScoreBasedGameKey(room.gameKey);
    roomParticipants(room).forEach((participant) => {
  ...
  if (botParticipant) {
    ...
    room.botFinishHandle = setTimeout(() => {
      if (botLeaveMs !== null) {
        ...
        // 729/Rakam Avı dahil ilk-bitiren botu kendi planlanmış finishMs anında tamamlar.
...
function resolveRealtimeRound(room) {
  ...
  const [first, second] = participants;
  if (isFirstFinishRaceGameKey(room.gameKey)) {
    const roundLimitMs = gameDefinition(room.gameKey).roundDurationMs;
    const firstElapsed = Number(first.roundElapsedMs ?? roundLimitMs);
    const secondElapsed = Number(second.roundElapsedMs ?? roundLimitMs);
    // Hedefi iki oyuncu da süre içinde tamamlayamadıysa kimlik sırasına göre sahte kazanan üretme.
    if (firstElapsed >= roundLimitMs && secondElapsed >= roundLimitMs) {
      first.totalElapsedMs += roundLimitMs;
      second.totalElapsedMs += roundLimitMs;
      finishRealtimeDraw(room, "target_not_reached");
      return;
    }
  }
  if (isRealtimeScoreBasedGameKey(room.gameKey)) {
...
function createRealtimeRoom(
  ...
  const baseGameKey = normalizeBaseGameKey(gameKey);
  const roundCount = (String(gameKey || "").endsWith("_tournament") || isFirstFinishRaceGameKey(baseGameKey))
    ? 1
